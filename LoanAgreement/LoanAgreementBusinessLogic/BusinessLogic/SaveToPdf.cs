...
namespace MaterialAccountingBusinessLogic.BusinessLogic
{
    public class SaveToPdf
    {
        public static void CreateDoc(PdfInfo info)
        {
            ...
            List<string> columns = new List<string> { "4cm", "6cm", "4cm", "4cm" };
            ...
            {
                Table = table,
                Texts = new List<string> { "Код материала", "Название материала",
"Количество", "Сумма" },
                Style = "NormalTitle",
                ParagraphAlignment = ParagraphAlignment.Center
            });

            foreach (var Operations in info.Operations)
            {
                CreateRow(new PdfRowParameters
                {
                    Table = table,
                    Texts = new List<string> { Operations.Code.ToString(), Operations.Name, Operations.Count.ToString(), Operations.Sum.ToString() },
                    Style = "Normal",
